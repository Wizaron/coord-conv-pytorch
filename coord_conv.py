...
if __name__ == '__main__':
    ...
    def test():
        ...
        input_image = torch.rand(batch_size, in_channels, image_height,
                                 image_width)
        input_image = Variable(input_image)
        if usegpu:
           input_image = input_image.cuda()

        coordinate_adder = AddCoordinates(with_r, usegpu)
        coord_adder_output = coordinate_adder(input_image)
        print 'Coordinates : ', coord_adder_output[batch_size - 1, :3]

        coord_conv = CoordConv(in_channels, out_channels, 3, with_r=with_r,
                               usegpu=usegpu)
        coord_conv_output = coord_conv(input_image)
        print 'CoordConv Input Size  : ', input_image.size()
        print 'CoordConv Output Size : ', coord_conv_output.size()

        from vgg16 import SkipVGG16, VGG16
        vgg = SkipVGG16()
        print vgg
        vgg = CoordConvNet(vgg, with_r=with_r, usegpu=usegpu)
        print vgg

        coordconvnet_output = vgg(input_image)
        print input_image.size()
        print coordconvnet_output.size()
